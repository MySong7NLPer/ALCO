...
def lenth_controll(summaries, abss, summaries_dual, ext_sents, reward_fn=compute_rouge_l):
    reward = reward_fn(summaries, abss) * reward_fn(summaries_dual, ext_sents) + reward_fn(summaries[:len(abss)], abss) - max(0,len(summaries)-len(abss))
    return reward

def a2c_train_step(agent, abstractor, abstractor_dual, loader, opt, grad_fn,
    ...
    with torch.no_grad():
        ...
        summaries_dual = abstractor_dual(summaries)
    i = 0
    rewards = []
    avg_reward = 0
    #print(len(indices))
    for inds, abss in zip(indices, abs_batch):
        ...
        # the basic reward
        rs = ([reward_fn(summaries[i+m], abss[m])
               for m in range(min(len(inds) - 1, len(abss)))]
        ...
        """ w/o ALC
        rs = ([reward_fn(summaries[i+m], abss[m]) * reward_fn(summaries_dual[i+m], ext_sents[m])
               for m in range(min(len(inds) - 1, len(abss)))]
              + [0 for _ in range(max(0, len(inds) - 1 - len(abss)))]
              + [stop_coeff * stop_reward_fn(
                    list(concat(summaries[i:i + len(inds) - 1])),
        ...
        """ w/o SEM with rho=1.0
        rs = ([reward_fn(summaries[i+m], abss[m]) - (max(0,len(summaries[i+m])-len(abss[m]))+0.0001)/len(abss[m])*reward_fn(summaries[i + m], abss[m])*1.0
               for m in range(min(len(inds) - 1, len(abss)))]
              + [0 for _ in range(max(0, len(inds) - 1 - len(abss)))]
              + [stop_coeff * stop_reward_fn(
                    list(concat(summaries[i:i + len(inds) - 1])),
        ...
        """ ALCO with rho=1.0
        rs = ([reward_fn(summaries[i+m], abss[m]) * reward_fn(summaries_dual[i+m], ext_sents[m]) - (max(0,len(summaries[i+m])-len(abss[m]))+0.0001)/len(abss[m])*reward_fn(summaries[i + m], abss[m])*1.0
               for m in range(min(len(inds) - 1, len(abss)))]
              + [0 for _ in range(max(0, len(inds) - 1 - len(abss)))]
              + [stop_coeff * stop_reward_fn(
                    list(concat(summaries[i:i + len(inds) - 1])),
...
class A2CPipeline(BasicPipeline):
    def __init__(self, name,
                 net, abstractor, abstractor_dual,
        ...
        self._abstractor_dual = abstractor_dual
        self._gamma = gamma
